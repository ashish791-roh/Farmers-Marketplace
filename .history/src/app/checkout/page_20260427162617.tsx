...
import { notifyUser, notifyFarmer, notifyAdmin } from "@/lib/notifications";
...
function CheckoutPageContent() {
  ...
  const grandTotal = Math.max(0, total - couponDiscount + delivery);
  ...
  const handleApplyCoupon = async (code?: string) => {
    ...
    try {
      const res = await fetch("/api/coupon/validate", {
        ...
        body: JSON.stringify({ code: codeToApply, orderTotal: total }),
      });
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            // ── Notify consumer ──────────────────────────────────────────
            const itemNames = cart.map((i: { name: string }) => i.name).join(", ");
            await notifyUser(user.uid, {
              type: "order_confirmed",
              title: "Order Confirmed ✅",
              message: `Your order for ${itemNames} has been placed successfully! Total: ₹${grandTotal}`,
              link: "/orders",
            });
            showPushNotification(
              "Order Confirmed! ✅",
              `Your FarmX order of ₹${grandTotal} is confirmed. We'll keep you updated!`,
              { tag: "order-confirmed", url: "/orders" }
            );

            // ── Notify each unique farmer ────────────────────────────────
            const farmerIds = new Set<string>(
              cart
                .filter((i: { farmerId?: string }) => i.farmerId)
                .map((i: { farmerId: string }) => i.farmerId)
            );
            for (const farmerId of farmerIds) {
              const farmerItems = cart
                .filter((i: { farmerId?: string }) => i.farmerId === farmerId)
                .map((i: { name: string; quantity: number }) => `${i.name} ×${i.quantity}`)
                .join(", ");
              await notifyFarmer(farmerId, {
                type: "new_order",
                title: "New Order Received 🛒",
                message: `You have a new order: ${farmerItems}`,
                link: "/farmer/orders",
              });
            }
            ...
            await notifyAdmin({
              ...
              message: `Order ${orderRef.id} placed by ${user.email} for ₹${grandTotal}`,
              link: "/admin/orders",
            });
                    ...
                    <div className="border border-dashed border-orange-200 rounded-xl p-4 bg-orange-50/50">
                      ...
                      {appliedCoupon ? (
                        <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2.5">
                          <div>
                            <span className="text-green-700 text-xs font-extrabold tracking-widest">{appliedCoupon.code}</span>
                            <p className="text-green-600 text-xs font-semibold mt-0.5">−₹{appliedCoupon.discountAmount} off applied!</p>
                          </div>
                          <button
                            type="button"
                        ...
                        <>
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white text-sm font-bold rounded-lg transition"
                            >
                              {couponLoading ? "..." : "Apply"}
                            </button>
                          </div>
            ...
            <div className="md:col-span-1">
              <div className="bg-white rounded-xl p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-6 border-b pb-4">
                  ...
                  {couponDiscount > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-orange-600 font-medium">Coupon ({appliedCoupon?.code})</span>
                      <span className="font-semibold text-orange-600">−₹{couponDiscount}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm">
