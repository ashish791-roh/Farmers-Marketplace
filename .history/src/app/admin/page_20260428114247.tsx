...
import { Package, Users, ShoppingCart, IndianRupee, Bell, Tractor } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubPending = onSnapshot(
      query(collection(db, "pendingProducts"), where("status", "==", "pending")),
      (snap) => {
        setStats((prev) => ({ ...prev, pendingProducts: snap.size }));
      }
    );
    ...
    const unsubFarmers = onSnapshot(
      ...
      (snap) => {
        setStats((prev) => ({ ...prev, pendingFarmers: snap.size }));
      }
    );
  ...
  const statCards = [
    ...
    { title: "Live Products",    value: stats.products, icon: <Package size={22} />,      color: "from-orange-500 to-orange-700" },
  ];
  ...
  return (
    <div>
      ...
      {(stats.pendingProducts > 0 || stats.pendingFarmers > 0) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          {stats.pendingProducts > 0 && (
            <Link href="/admin/notifications">
              <motion.div
                whileHover={{ scale: 1.02 }}
                className="flex items-center gap-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-2xl cursor-pointer hover:border-yellow-500/50 transition"
              >
                ...
                <div>
                  <p className="font-semibold text-yellow-400">
                    {stats.pendingProducts} Product{stats.pendingProducts !== 1 ? "s" : ""} Awaiting Review
                  </p>
                  <p className="text-xs text-gray-400 mt-0.5">Click to review and approve/reject</p>
                </div>
          ...
          {stats.pendingFarmers > 0 && (
            <Link href="/admin/farmers">
              <motion.div
                whileHover={{ scale: 1.02 }}
                className="flex items-center gap-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-2xl cursor-pointer hover:border-blue-500/50 transition"
              >
                <div className="bg-blue-500/20 p-3 rounded-xl">
                  <Tractor size={22} className="text-blue-400" />
                </div>
                <div>
                  <p className="font-semibold text-blue-400">
                    {stats.pendingFarmers} Farmer{stats.pendingFarmers !== 1 ? "s" : ""} Awaiting Approval
                  </p>
                  <p className="text-xs text-gray-400 mt-0.5">Review new farmer registrations</p>
                </div>
