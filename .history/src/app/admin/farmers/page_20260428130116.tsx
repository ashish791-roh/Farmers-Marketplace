...
export default function AdminFarmers() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const sorted = snap.docs
        .map((d) => ({ id: d.id, ...d.data() } as Farmer))
        .sort((a, b) => {
  ...
  const handleApprove = async (farmer: Farmer) => {
    ...
    try {
      ...
      toast.success(`${farmer.displayName || farmer.email} approved!`);
    } catch {
  ...
  const handleReject = async (farmer: Farmer) => {
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-500 to-emerald-700 flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
                  {(farmer.displayName || farmer.email || "F")[0].toUpperCase()}
                </div>

                {/* INFO */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    <h3 className="font-semibold text-white">{farmer.displayName || "No Name"}</h3>
