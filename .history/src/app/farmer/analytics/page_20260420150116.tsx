...
export default function FarmerAnalytics() {
  ...
  useEffect(() => {
    ...
    const unsubOrders = onSnapshot(collection(db, "orders"), (snap) => {
      const farmerOrders = snap.docs
        ...
        .filter((o: any) => o.items?.some((i: any) => i.farmerId === user.uid));
  ...
  const totalRevenue = orders.reduce((sum, order) => {
    const myItems = order.items?.filter((i: any) => i.farmerId === user?.uid) || [];
    return sum + myItems.reduce((s: number, i: any) => s + (i.price * i.quantity), 0);
  }, 0);

  const totalUnitsSold = orders.reduce((sum, order) => {
    const myItems = order.items?.filter((i: any) => i.farmerId === user?.uid) || [];
    return sum + myItems.reduce((s: number, i: any) => s + (i.quantity || 0), 0);
  }, 0);
  ...
  orders.forEach((order) => {
    order.items
      ?.filter((i: any) => i.farmerId === user?.uid)
      .forEach((item: any) => {
