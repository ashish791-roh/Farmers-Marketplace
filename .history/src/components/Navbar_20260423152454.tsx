...
import LocationPicker, { useDeliveryLocation } from "@/components/LocationPicker";
import {
  ShoppingCart,
  Heart,
  User,
...
export default function Navbar() {
  ...
  const { location, updateLocation } = useDeliveryLocation();
  ...
  const locationLabel = location.city.length > 12 ? location.city.slice(0, 12) + "…" : location.city;
  const pincodeLabel = location.pincode || "Set PIN";
      ...
      <div className={`w-full sticky top-0 z-50 transition-shadow duration-200 ${scrolled ? "shadow-lg" : "shadow-sm"}`}>
        ...
        <div className="bg-green-700 text-white overflow-hidden">
          <div className="max-w-7xl mx-auto px-3 md:px-6 py-1.5 flex items-center justify-between gap-4">
            ...
            {/* Top-strip location (desktop) — clickable */}
            <button
              onClick={() => setLocationOpen(true)}
              className="hidden md:flex items-center gap-1 shrink-0 text-[11px] text-green-200 hover:text-white transition group"
            >
              <MapPin size={11} />
              <span>
                Deliver to:{" "}
        ...
        <nav className="bg-white border-b border-gray-100">
          ...
          <div className="max-w-7xl mx-auto px-3 md:px-6">
            <div className="flex items-center gap-2 md:gap-4 h-14 md:h-16">
              ...
              {/* ── LOCATION BUTTON (desktop) — opens picker ── */}
              <button
                onClick={() => setLocationOpen(true)}
              ...
              <div className="flex-1 max-w-xl mx-1 md:mx-2">
                ...
                <div className="md:hidden flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setLocationOpen(true)}
                    className="flex items-center gap-1 px-2.5 py-2 rounded-xl bg-gray-50 border border-gray-200 shrink-0 active:bg-gray-100 transition"
                  >
                    <MapPin size={13} className="text-green-600" />
                    <span className="text-[11px] font-bold text-gray-700 max-w-[70px] truncate">{locationLabel}</span>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 flex-1 px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-400 active:bg-gray-100 transition"
                  >
                    <Search size={15} />
                          ...
                          {user ? (
                            <>
                              <div className="px-4 py-3 bg-gradient-to-br from-green-50 to-emerald-50 border-b border-gray-100">
                                ...
                                {/* Show location in profile dropdown too */}
                                <button
                                  onClick={() => { setProfileOpen(false); setLocationOpen(true); }}
                                  className="mt-2 w-full flex items-center gap-1.5 text-[11px] text-gray-500 hover:text-green-600 transition"
                                >
                                  <MapPin size={11} />
                                  <span className="truncate">{location.city}, {location.pincode || "Set pincode"}</span>
                                  <span className="text-green-500 ml-auto font-medium">Change</span>
                                </button>
                              </div>
